...
@logger.catch
def check_and_retrieveVocabulary(YSequences, vocab_path):
    w2ipath = vocab_path + "w2i.npy"
    i2wpath = vocab_path + "i2w.npy"

    w2i, i2w = make_vocabulary(YSequences, vocab_path)

    return w2i, i2w

def make_vocabulary(YSequences, vocab_path):
    vocabulary = set()
    ...
    #Save the vocabulary
    np.save(vocab_path + "w2i.npy", w2i)
    np.save(vocab_path + "i2w.npy", i2w)
